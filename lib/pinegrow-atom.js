...
export default {
  ...
  initEditorApi() {
    ...
    if(!urlinfo.hostname || !urlinfo.port) {
        atom.notifications.addError('Hostname and port are not set in Pinegrow URL.');
        return;
    }
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'pinegrow-atom:open-in-pg': () => this.openInPinegrow(),
      'pinegrow-atom:reconnect': () => this.reconnect(),
      'pinegrow-atom:detect-mapping': () => this.autoDetectMapping(),
      'pinegrow-atom:download-pinegrow': () => this.downloadPinegrow(),
      'pinegrow-atom:refresh-page': () => this.refreshPage()
    }));
...
*/
    ...
    atom.workspace.observeTextEditors(function(editor) {
        editor.onDidStopChanging(function(obj) {
            if(obj && obj.changes && obj.changes.length) {
  ...
  selectInPinegrow(event) {
    ...
    if(editor) {
        var url = getPgEditorUrl(editor);
        if(true || this.isFileOpenInPinegrow(url)) {
...
openInPinegrow(event) {
    var editor = atom.workspace.getActiveTextEditor();
    if(editor) {
        var url = getPgEditorUrl(editor);
        editor_api.emit('openFile', {url: url});
    }
  },
...
refreshPage(event) {
    ...
    if(editor) {
      var url = getPgEditorUrl(editor);
      editor_api.emit('refreshPage', {url: url});
    }
},
  ...
  downloadPinegrow() {
      ...
      shell.openExternal('http://pinegrow.com');
  }
